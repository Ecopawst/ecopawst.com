...
export default function Upload() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await fetch('/api/report-bug', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ type: 'UploadSaveError', message: err.message, stack: err.stack, context: { petId, groupId } })
      });
      console.error(err);
      setMessage('Upload failed.');
    } finally {
  ...
  return (
    <form onSubmit={handleSubmit} className="p-8 space-y-4">
      ...
      <label className="block">Post to Group
        <select className="border p-2 w-full" value={groupId} onChange={e => setGroupId(e.target.value)}>
